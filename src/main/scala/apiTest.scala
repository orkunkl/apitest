    ...
    val tryParsing = for{
      ...
      val tryParsingToObjects = for{
        ...
        if(resApiJsonValueSize != apiJsonValueSize)
          println("sizes do not match! Resource api size = " + resApiJsonValueSize + " api size = " + apiJsonValueSize + "\n")

        println(resApiJson)

        resApiJsonValue.head.hcursor.fields.get.map(field => apiJsonValue.head.asObject.get.apply(field) match {
          case None => println(field + " not found in api")
          case Some(a) => ()
        })
      }
      tryParsingToObjects.getOrElse(println("There has been an error with parsing json to objects"))
    }
